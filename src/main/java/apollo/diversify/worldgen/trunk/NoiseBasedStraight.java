package apollo.diversify.worldgen.trunk;
...
public class NoiseBasedStraight extends TrunkPlacer {
    public static final Codec<NoiseBasedStraight> CODEC = RecordCodecBuilder.create((instance) -> {
        return instance.group(
                ...
                }), SeededNoiseProvider.MODIFIER_CODEC.fieldOf("noise_provider").forGetter((trunkPlacer) -> {
    ...
    protected TrunkPlacerType<?> getType() {
        return TrunkPlacerRegistry.NOISE_BASED_STRAIGHT;
    }
    ...
    public List<FoliagePlacer.TreeNode> generate(TestableWorld world, BiConsumer<BlockPos, BlockState> replacer, Random random, int height, BlockPos startPos, TreeFeatureConfig config) {
        ...
        int j = Math.toIntExact(Math.min(Math.max(Math.round(((this.maximumHeight-this.minimumHeight)/2)*i+((this.minimumHeight+this.maximumHeight)/2)),this.minimumHeight),this.maximumHeight))+ random.nextInt(this.randomHeight+1);
